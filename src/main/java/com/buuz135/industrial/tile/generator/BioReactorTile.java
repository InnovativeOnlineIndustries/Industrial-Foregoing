...
public class BioReactorTile extends CustomElectricMachine {
    ...
    protected float performWork() {
        ...
        if (tank.getFluid() == null || (stack.amount + tank.getFluidAmount() <= tank.getCapacity())){
            ...
            for (int i = 0; i < buffer.getSlots(); ++i) {
                if (!buffer.getStackInSlot(i).isEmpty()) buffer.getStackInSlot(i).setCount(buffer.getStackInSlot(i).getCount()-1);
            }
            return 1;
        }
        return 0;
    ...
    private boolean alreadyContains(ItemStackHandler handler, ItemStack stack, int amountAtleast){
        for (int i = 0; i < handler.getSlots(); ++i){
            if (stack.getItem().equals(handler.getStackInSlot(i).getItem()) && stack.getMetadata() == handler.getStackInSlot(i).getMetadata() && handler.getStackInSlot(i).getCount() >= amountAtleast) return true;
        }
        return false;
    }
